class ArticlesController < ApplicationController
  ...
  def authored
    ...
    @articles = KbArticle.where("#{KbArticle.table_name}.id in (?)", @project.articles.where(:author_id => @author_id).map(&:id))
    ...
    if params[:tag]
      ...
      @articles = KbArticle.where("#{KbArticle.table_name}.id in (?)", @articles.tagged_with(@tag).map(&:id))
    end
